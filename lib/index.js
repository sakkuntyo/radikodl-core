 ...
 */
const ytdl = (link, token, options) => {
  const stream = createStream(options);
  downloadFromInfoCallback(stream, link, token, options);
  return stream;
};
module.exports = ytdl;
...
const downloadFromInfoCallback = (stream, link, token, options) => {
  options = options || {};
  ...
  options.requestOptions.headers["X-Radiko-Authtoken"] = token;
  if (format.isHLS || format.isDashMPD) {
    ...
    if (shouldBeChunked) {
      ...
      const getNextChunk = () => {
        ...
        req = miniget(link + "/", requestOptions);
        req.on('data', ondata);
        req.on('end', () => {
      ...
      if (options.begin) {
        format.url = link + `&begin=${parseTimestamp(options.begin)}`;
      }
